...
BACKUP_LOCATIONS = {
    ...
    "backup_test3": "gdrive:/Backups/-Orion-/Test_3/",
    "backup_test4": "gdrive:/Backups/-Orion-/Test_4/",
}

# Define rclone arguments (only one per line)
ARGS = [
    "--transfers=10", 
    "--progress",
    "--verbose"
...
def rclone_sync(source, destination, args=None):
    ...
    try:
        log_data = subprocess.run(command, capture_output=True, text=True, check=True)
        logger.info(f"Sync completed for {source} to {destination}")
        logger.debug(f"stdout: {log_data.stdout}")
        logger.debug(f"stderr: {log_data.stderr}")
